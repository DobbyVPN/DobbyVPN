...
class DesktopBuildTests(unittest.TestCase):
    ...
    def test_android_ndk_provider_ignores_blank_environment_values(self) -> None:
        ...
        self.assertIn('manifestPlaceholders["dobbyVersionCode"] = releaseVersionCode.toString()', android)
        self.assertIn('manifestPlaceholders["dobbyVersionName"] = releaseVersionName', android)
        self.assertIn('val goBuildRoot = layout.buildDirectory.dir("generated/go-build")', android)
        self.assertIn('environment("GOENV", "off")', android)
        self.assertIn('environment("GOTOOLCHAIN", "local")', android)
        self.assertIn('environment("GOTMPDIR", goTemp.absolutePath)', android)
    ...
    def test_desktop_packaging_uses_native_inputs_and_no_conveyor(self) -> None:
        ...
        self.assertIn("path: runtime/services", workflow)
        self.assertIn("path: runtime/services/macos-amd64", workflow)
