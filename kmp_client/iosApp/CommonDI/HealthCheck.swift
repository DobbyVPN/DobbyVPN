...
public class HealthCheck {
    ...
    public func fullCheckUp() {
        do {
            ...
            try safeRepeat(repeatCount) {
                self.httpGetBody(urlString: "https://google.com/gen_204") { success, body, errorMessage in
                    if success {
                        self.logs.writeLog(log: "[httpGetBody] GET request successful (https://google.com/gen_204)!")
                    } else {
                        self.logs.writeLog(log: "[httpGetBody] GET request failed (https://google.com/gen_204). Error: \(errorMessage ?? "Unknown")")
                    }
                }
            }
