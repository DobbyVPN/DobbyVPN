...
func TestConfigureCompatibilityProfileClonesAndIsIdempotent(t *testing.T) {
	m := NewManager(ManagerOptions{})
	id, err := m.CreateSession(context.Background())
	if err != nil {
		t.Fatal(err)
	...
	result, err := m.ConfigureCompatibilityProfile(context.Background(), id, "legacy-configure", profile)
	if err != nil {
		t.Fatal(err)
	}
	...
	again, err := m.ConfigureCompatibilityProfile(context.Background(), id, "legacy-configure", RuntimeProfile{Summary: ProfileSummary{Protocol: ProtocolOutline}, NormalizedFormat: ConfigTransportURL, NormalizedConfig: []byte("different")})
	if err != nil || again.Digest != result.Digest {
		t.Fatalf("idempotent compatibility configure = %#v, %v", again, err)
	}
}

func TestConfigureCompatibilityProfileValidatesFormatAndActiveGeneration(t *testing.T) {
	for _, profile := range []RuntimeProfile{
		...
		if _, err := m.ConfigureCompatibilityProfile(context.Background(), id, "bad", profile); CodeOf(err) != FailureInvalidArgument {
	...
	waitState(t, m, id, StateConnected)
	if _, err := m.ConfigureCompatibilityProfile(context.Background(), id, "configure-active", profile); CodeOf(err) != FailureConflict {
