...
func StartRoutingCloak(proxyIP string) error {
	...
	if err != nil {
		...
		if err != nil {
			log.Warnf(Category, "Cloak/routing: Failed to add or update proxy route for IP %s: %v", log.MaskStr(proxyIP), err)
		}
	}
	return nil
}
...
func StopRoutingCloak(proxyIp string) {
	...
	_, err := routing.ExecuteCommand(command)
	if err != nil {
		log.Warnf(Category, "Cloak/routing: Failed to delete proxy route for IP %s: %v\n", log.MaskStr(proxyIp), err)
	}
	log.Infof(Category, "Cloak/routing: Cleaned up routing table and rules.")
}
