...
func (app App) Run(ctx context.Context, initResult chan<- error) error {
	log.Infof(Category, "[Linux][Init] ===== VPN initialization started =====")
	...
	if err != nil {
		...
		log.Errorf(Category, "[Linux][Step 1][ERROR] %v", err)
	...
	log.Debugf(Category, "[Linux][Step 2] Resolving VPN server IP...")
	serverIP, err := ResolveServerIPFromConfig(*app.TransportConfig)
	if err != nil {
		err = fmt.Errorf("failed to resolve server IP from config: %w", err)
		log.Errorf(Category, "[Linux][Step 2][ERROR] %v", err)
		signalInit(initResult, err)
		return err
	}
	log.Debugf(Category, "[Linux][Step 2][OK] ServerIP=%s", serverIP.String())
	...
	if err != nil {
		...
		log.Errorf(Category, "[Linux][Step 3][ERROR] %v", err)
		signalInit(initResult, err)
		return err
	}
	...
	if serverIP.String() != "127.0.0.1" {
		...
		if err = routing.AddProxyRoute(serverIP.String(), gatewayIP.String(), uplinkIface); err != nil {
			...
			log.Errorf(Category, "[Linux][Step 4][ERROR] %v", err)
			signalInit(initResult, err)
			return err
		}
		common.Client.MarkOutOffCriticalSection(outlineCommon.Name)
	...
	if err = routing.SetupMarkedRouting(
		...
		log.Errorf(Category, "[Linux][Step 5][ERROR] %v", err)
		signalInit(initResult, err)
		return err
	}
	common.Client.MarkOutOffCriticalSection(outlineCommon.Name)
	...
	if err != nil {
		...
		log.Errorf(Category, "[Linux][Step 6][ERROR] %v", err)
		signalInit(initResult, err)
		return err
	}
	...
	// 7. Outline
	log.Debugf(Category, "[Linux][Step 7] Creating Outline SOCKS bridge...")
	ss, err := NewOutlineDevice(*app.TransportConfig)
	if err != nil {
		_ = tun.Close()
		err = fmt.Errorf("failed to create OutlineDevice: %w", err)
		log.Errorf(Category, "[Linux][Step 7][ERROR] %v", err)
		signalInit(initResult, err)
		return err
	}
	log.Debugf(Category, "[Linux][Step 7][OK] SOCKS5 proxy=%s", ss.GetProxyAddr())
	...
	closeAll := func() {
		closeOnce.Do(func() {
			...
			if err = routing.StopRouting(serverIP.String(), gatewayIP.String(), uplinkIface); err != nil {
				log.Warnf(Category, "[Linux][StopRouting][ERROR] %v", err)
			}
			common.Client.MarkOutOffCriticalSection(outlineCommon.Name)
			...
			if err = routing.CleanupMarkedRouting(
				...
				log.Warnf(Category, "[Linux][CleanupMarkedRouting][WARN] %v", err)
			}
			...
			log.Infof(Category, "[Linux][Lifecycle] Shutdown complete")
		})
	}
	...
	if fd < 0 {
		...
		log.Errorf(Category, "[Linux][Step 8][ERROR] %v", err)
		signalInit(initResult, err)
		return err
	}
	...
	// 9. tun2socks
	log.Debugf(Category, "[Linux][Step 9] Starting tun2socks (fd=%d proxy=%s)", fd, ss.GetProxyAddr())
	...
	if err = routing.StartRouting(serverIP.String(), gatewayIP.String(), uplinkIface, app.RoutingConfig.TunDeviceName); err != nil {
		...
		log.Errorf(Category, "[Linux][Step 10][ERROR] %v", err)
		signalInit(initResult, err)
		return err
	}
	common.Client.MarkOutOffCriticalSection(outlineCommon.Name)
	...
	log.Infof(Category, "[Linux][Init] ===== VPN started successfully =====")
	...
	log.Infof(Category, "[Linux][Lifecycle] Context cancelled — stopping engine")
	return nil
}
