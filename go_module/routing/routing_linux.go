...
// session's policy rule remains managed by the routing plan; this compatibility
// wrapper preserves the original route-only API for callers that do not own a
// rule priority.
func ReconcileLinuxSessionRoutes(proxyIP, gatewayIP, iface string, tableID int) error {
	return reconcileLinuxSessionRoutes(proxyIP, gatewayIP, iface, tableID, 0)
}
