...
func (c *trackedConn) Close() error {
	c.once.Do(func() {
		...
		if active%10 == 0 {
			log.Debugf(Category, "[Router] TCP closed: activeTCP=%d/%d", active, maxActiveTCPConns)
		}
	})
	return c.Conn.Close()
}
...
func (c *trackedPacketConn) Close() error {
	...
	c.once.Do(func() {
		close(c.done)
		active := c.counter.Add(-1)
		log.Debugf(Category, "[Router] UDP closed (idle): activeUDP=%d/%d", active, maxActiveUDPConns)
...
func (p *DobbyProxy) DialContext(ctx context.Context, metadata *M.Metadata) (net.Conn, error) {
	...
	log.Debugf(Category, "[Router] TCP dial attempt to %s proto=%s", destAddr, metadata.Network)

	if IsBypass(metadata) {
		conn, err := p.direct.DialContext(ctx, metadata)
		if err != nil {
			log.Debugf(Category, "[Router] BYPASS TCP dial error %s: %v", metadata.DestinationAddress(), err)
			return nil, err
		}
		log.Debugf(Category, "[DEBUG][Router] BYPASS TCP dial OK %s local=%s remote=%s", metadata.DestinationAddress(), conn.LocalAddr(), conn.RemoteAddr())
		return conn, nil
	}
	...
	if active > maxActiveTCPConns {
		p.activeTCP.Add(-1)
		log.Debugf(Category, "[Router] TCP dropped (activeTCP=%d): %s", active-1, metadata.DestinationAddress())
	...
	if err != nil {
		...
		log.Debugf(Category, "[Router] PROXY TCP dial error %s: %v", metadata.DestinationAddress(), err)
		return nil, err
	}

	if active%10 == 0 {
		log.Debugf(Category, "[Router] pool: activeTCP=%d/%d activeUDP=%d/%d", active, maxActiveTCPConns, p.activeUDP.Load(), maxActiveUDPConns)
	}
...
func (p *DobbyProxy) DialUDP(metadata *M.Metadata) (net.PacketConn, error) {
	if IsBypass(metadata) {
		...
		if err != nil {
			log.Debugf(Category, "[Router] BYPASS UDP dial error %s: %v", metadata.DestinationAddress(), err)
			return nil, err
		}
		log.Debugf(Category, "[DEBUG][Router] BYPASS UDP dial OK %s local=%s", metadata.DestinationAddress(), conn.LocalAddr())
		return conn, nil
	}

	// iOS 26 diagnostic: log UDP connection details
	destAddr := metadata.DestinationAddress()
	log.Debugf(Category, "[Router] UDP dial attempt to %s proto=%s", destAddr, metadata.Network)

	active := p.activeUDP.Add(1)
	if active > maxActiveUDPConns {
		p.activeUDP.Add(-1)
		log.Debugf(Category, "[Router] UDP dropped (activeUDP=%d): %s", active-1, metadata.DestinationAddress())
	...
	if err != nil {
		p.activeUDP.Add(-1)
		log.Debugf(Category, "[Router] PROXY UDP dial error %s: %v", metadata.DestinationAddress(), err)
		return nil, err
	}

	if active%10 == 0 {
		log.Debugf(Category, "[Router] pool: activeTCP=%d/%d activeUDP=%d/%d", p.activeTCP.Load(), maxActiveTCPConns, active, maxActiveUDPConns)
	}
...
func StartEngine(cfg platform_engine.EngineConfig) error {
	...
	log.Debugf(Category, "[Engine] StartEngine config proxy=%s fd=%d mtu=%d uplinkIface=%s", cfg.ProxyAddr, cfg.FD, cfg.MTU, cfg.UplinkIface)
	log.Debugf(Category, "[Engine] StartEngine: calling StartPlatformEngine")
	...
	t := tunnel.T()
	if t == nil {
		log.Debugf(Category, "[Engine] tunnel.T() is nil after engine start — tun2socks did not initialise")
	...
	t.SetDialer(wrapper)
	log.Debugf(Category, "[Engine] DobbyProxy installed (maxTCP=%d maxUDP=%d)", maxActiveTCPConns, maxActiveUDPConns)
	isRunning = true
	return nil
}
