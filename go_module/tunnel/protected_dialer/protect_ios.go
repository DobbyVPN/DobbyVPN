...
func (i *iosProtector) Protect(fd uintptr, network string) error {
	...
	if err != nil {
		log.Debugf(Category, "[iOS-Protect] SO_NO_TC_NETPOLICY failed: %v", err)
	}
	...
	if boundIfErr != nil {
		...
		log.Debugf(Category, "[iOS-Protect] IP_BOUND_IF not available: %v (this is normal on some iOS versions)", boundIfErr)
	}
...
func init() {
	...
	log.Debugf(Category, "[iOS-Protect] Initialized with iOS 26 research options (SO_NO_TC_NETPOLICY + IP_BOUND_IF research)")
}
