...
func DialContextWithProtect(ctx context.Context, network, address string) (net.Conn, error) {
	...
	log.Debugf(Category, "[Protect] TCP dial OK: dest=%s local=%s remote=%s", address, localAddr, conn.RemoteAddr())

	// iOS 26 research: Warn if connection is NOT going through tunnel
	if !strings.HasPrefix(localAddr, "198.18.") {
		log.Debugf(Category, "[Protect] *** CRITICAL *** Outbound TCP connection NOT using VPN tunnel! local=%s - THIS IS THE PROBLEM ON iOS 26", localAddr)
	}
	return conn, nil
}
...
func DialUDPWithProtect(ctx context.Context, network, address string) (net.PacketConn, error) {
	...
	if !strings.HasPrefix(localAddr, "198.18.") {
		log.Debugf(Category, "[Protect] *** CRITICAL *** Outbound UDP connection NOT using VPN tunnel! local=%s - THIS IS THE PROBLEM ON iOS 26", localAddr)
	}
	return &connectedUDPConn{
		PacketConn: pc,
		remoteAddr: udpAddr,
