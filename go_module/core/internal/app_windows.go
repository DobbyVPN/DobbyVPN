...
func (app App) Run(ctx context.Context, initResult chan<- error) error {
	...
	if err != nil {
		...
		log.Debugf(Category, "[Routing] %v", err)
		signalInit(initResult, err)
		return err
	}
	...
	log.Debugf(Category, "[Windows] Proxy addr: %s", app.ProtocolDevice.GetProxyAddr())
	...
	log.Debugf(Category, "[Routing] Routing successfully configured")
	...
	log.Debugf(Category, "Core/app: received interrupt signal, terminating...")
