...
func (app App) Run(ctx context.Context, initResult chan<- error) error {
	...
	log.Debugf(Category, "[Protocol] ProtocolDevice successfully created")
	...
	if err != nil {
		...
		if err := routing.AddScopedDefaultRoute(ifaceName, gatewayIP.String()); err != nil {
			log.Debugf(Category, "[Routing] ERROR: failed to add scoped default route via %s: %v", ifaceName, err)
		} else {
	...
	log.Debugf(Category, "[Lifecycle] VPN initialization completed successfully")

	signalInit(initResult, nil)

	<-ctx.Done()

	log.Debugf(Category, "[Lifecycle] Context cancelled — stopping VPN engine")
