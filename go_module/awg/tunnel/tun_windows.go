...
func (a *TunnelData) Run() error {
	...
	log.Debugf(Category, "Running awg tunnel (windows)")
	...
	if err != nil {
		log.Debugf(Category, "[WARNING] unable to determine Wintun version: %v", err)
	} else {
...
func (a *TunnelData) tunnelLoop() {
	...
	select {
	case err := <-a.errs:
		log.Debugf(Category, "[ERROR] Got error, stopping tunnel loop: %s", err)
		return
	case <-a.dev.Wait():
		log.Debugf(Category, "[WARNING] Device wait call, stopping tunnel loop")
		return
	case err := <-a.watcher.errors:
		log.Debugf(Category, "[ERROR] Got watcher error, stopping tunnel loop: %s", err)
		return
	}
}
