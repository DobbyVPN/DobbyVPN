...
func protector(network string, address string, c syscall.RawConn) error {
	...
	controlErr := c.Control(func(fd uintptr) {
		...
		if err := syscall.SetsockoptInt(int(fd), syscall.SOL_SOCKET, SO_NO_TC_NETPOLICY, 1); err != nil {
			log.Infof("Cloak protect SO_NO_TC_NETPOLICY failed (expected on iOS 26+): fd=%d err=%v", fd, err)
		}
		...
		if ifaceIndex > 0 {
			...
			protectErr = fmt.Errorf("no default interface index set for Cloak socket protection")
		}
	})
