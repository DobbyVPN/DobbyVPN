...
func (c *CkClient) Connect() (returnErr error) {
	defer func() {
		if r := recover(); r != nil {
			log.Warnf(Category, "ck-client Connect: recovered from panic: %v", r)
	...
	c.connected = true
	log.Debugf(Category, "ck-client connected")
	...
	if len(c.config.UID) != 0 {
		...
		log.Debugf(Category, "ck-client: adminUID set to %s", adminUID)
	}
	...
	if adminUID != nil {
		log.Debugf(Category, "API base is %v", localConfig.LocalAddr)
		authInfo.UID = adminUID
		authInfo.SessionId = 0
		remoteConfig.NumConn = 1

		log.Debugf(Category, "Before seshMaker")
		seshMaker = func() *mux.Session {
			log.Debugf(Category, "In seshMaker")
		...
		log.Infof(Category, "ck-client: Listening on %v %v for %v client", network, localConfig.LocalAddr, authInfo.ProxyMethod)
		seshMaker = func() *mux.Session {
	...
	go func() {
		...
		defer func() {
			if r := recover(); r != nil {
				...
				log.Warnf(Category, "ck-client: recovered from panic from: %v", r)
			}
		}()
		if authInfo.Unordered {
			...
			if err != nil {
				log.Errorf(Category, "ck-client: goroutines: err %v\n", err)
				return
			}

			c.udpConn = conn

			log.Infof(Category, "ck-client: start listening on UDP %v for %v client", localConfig.LocalAddr, authInfo.ProxyMethod)
			client.RouteUDP(func() (*net.UDPConn, error) { return conn, nil }, localConfig.Timeout, remoteConfig.Singleplex, seshMaker)
			log.Infof(Category, "ck-client: stop listening on UDP %v for %v client", localConfig.LocalAddr, authInfo.ProxyMethod)
		} else {
			...
			log.Infof(Category, "ck-client: start listening on TCP %v for %v client", localConfig.LocalAddr, authInfo.ProxyMethod)
			client.RouteTCP(l, localConfig.Timeout, remoteConfig.Singleplex, seshMaker)
			log.Infof(Category, "ck-client: stop listening on TCP %v for %v client", localConfig.LocalAddr, authInfo.ProxyMethod)
		}
	}()
...
func (c *CkClient) Disconnect() error {
	...
	if !c.connected {
		log.Infof(Category, "ck-client: already disconnected")
		return nil
	}

	log.Debugf(Category, "ck-client: initiating disconnect...")
	c.connected = false

	if c.listener != nil {
		addr := c.listener.Addr().String()
		if err := c.listener.Close(); err != nil {
			log.Warnf(Category, "ck-client: error closing TCP listener %v: %v", addr, err)
		} else {
			log.Infof(Category, "ck-client: TCP listener %v closed", addr)
		}
		c.listener = nil
	}
	...
	if c.udpConn != nil {
		...
		if err := c.udpConn.Close(); err != nil {
			log.Warnf(Category, "ck-client: error closing UDP conn %v: %v", addr, err)
		} else {
			log.Infof(Category, "ck-client: UDP listener %v closed", addr)
		}
		c.udpConn = nil
	}

	if c.session != nil {
		log.Debugf(Category, "ck-client: closing session...")
		c.session.Close()
		c.session = nil
		log.Infof(Category, "ck-client: session closed")
	}
	...
	if c.routeDone != nil {
		...
		case <-time.After(2 * time.Second):
			log.Warnf(Category, "ck-client: routing goroutine did not exit within timeout")
		}
		c.routeDone = nil
	}

	log.Infof(Category, "ck-client: fully disconnected")
	return nil
}
