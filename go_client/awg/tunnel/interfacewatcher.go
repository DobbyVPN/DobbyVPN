...
func hasDefaultRoute(family winipcfg.AddressFamily, peers []config.Peer) bool {
	var (
		foundV401    bool
		foundV41281  bool
		foundV600001 bool
...
func (iw *interfaceWatcher) Configure(binder conn.BindSocketToInterface, conf *config.Config, tun *tun.NativeTun) {
	iw.setupMutex.Lock()
	defer iw.setupMutex.Unlock()
